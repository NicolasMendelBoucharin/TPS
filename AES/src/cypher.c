#include "cypher.h"
#include "AES.h"

int Nk= 4;//key length
int Nb=4; //block size
int Nr= 10;//number of rounds

//Sbox du papier
const uint8_t sbox[256] = {
    
};


const uint8_t inv_sbox[256] = {
    
};


//je m'en sers pas au final mais j'ai laissé pour avoir les valeurs noté
const uint8_t multiplication_galois[16] = { 
    0x02, 0x01, 0x01, 0x03,
    0x03, 0x02, 0x01, 0x01,
    0x01, 0x03, 0x02, 0x01,
    0x01, 0x01, 0x03, 0x02
};

const uint8_t multiplication_galoix_inv[16] = { 
    0x0e, 0x09, 0x0d, 0x0b,
    0x0b, 0x0e, 0x09, 0x0d,
    0x0d, 0x0b, 0x0e, 0x09,
    0x09, 0x0d, 0x0b, 0x0e
};

//fonctions auxilliaires pour être sûr de pas faire de de modif
uint8_t get_sbox_value(uint8_t n) {
    return sbox[n];
}

uint8_t get_inv_sbox_value(uint8_t n) {
    return inv_sbox[n];
}

//on applique à chaque cellule la transformation de la sbox
void sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = get_sbox_value(state[i]);
    }
}

//on applique à chaque cellule la transformation de la invsbox
void inv_sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = get_inv_sbox_value(state[i]);
    }
}

//faut voir mes listes comme ça:
//0, 4, 8, 12
//1, 5, 9, 13
//2, 6, 10, 14
//3, 7, 11, 15
//je me suis compliqué la vie de zinzin à faire avec des listes plutot que des matrices
void shift_row(uint8_t* state){
    uint8_t temp[16];
    for (int i = 0; i < 16; i++) {
        temp[i] = state[i];
    }

    state[1] = temp[5];
    state[5] = temp[9];
    state[9] = temp[13];
    state[13] = temp[1];

    state[2] = temp[10];
    state[6] = temp[14];
    state[10] = temp[2];
    state[14] = temp[6];

    
    state[3] = temp[15];
    state[7] = temp[3];
    state[11] = temp[7];
    state[15] = temp[11];
}

void inv_shift_row(uint8_t* state) {
    uint8_t temp[16];
    for (int i = 0; i < 16; i++) {
        temp[i] = state[i];
    }

    state[1] = temp[13];
    state[5] = temp[1];
    state[9] = temp[5];
    state[13] = temp[9];

    state[2] = temp[10];
    state[6] = temp[14];
    state[10] = temp[2];
    state[14] = temp[6];

    state[3] = temp[7];
    state[7] = temp[11];
    state[11] = temp[15];
    state[15] = temp[3];
}

//comme dans le TP3 de crypto
uint8_t foisX(uint8_t a) {
    uint8_t result = a << 1; 
    uint8_t poids_fort = ((a>> 7) & 0x01);
    if (poids_fort == 1) {
        result ^= 0x1b; 
    }
    return result;
}

uint8_t mul(uint8_t a , uint8_t b){
    char S=a;
    char t= 0x00;
    
    for (int i = 0; i < 8; i++) {
        if (((b>>i)& 0x01) == 1) {
            t ^= S; 
        }
        S = foisX(S); 
        
    }
    return t;
}

//comme dans le papier avec un truc temp pour être sur de pas modifier des mauvais trucs si je fais pas dans le bon ordre
void mix_column(uint8_t *state) {
    uint8_t temp[4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            temp[j] = state[i * 4 + j];
        }
        state[i * 4] = (uint8_t)(mul(0x02, temp[0]) ^ mul(0x03, temp[1]) ^ temp[2] ^ temp[3]);
        state[i * 4 + 1] = (uint8_t)(temp[0] ^ mul(0x02, temp[1]) ^ mul(0x03, temp[2]) ^ temp[3]);
        state[i * 4 + 2] = (uint8_t)(temp[0] ^ temp[1] ^ mul(0x02, temp[2]) ^ mul(0x03, temp[3]));
        state[i * 4 + 3] = (uint8_t)(mul(0x03, temp[0]) ^ temp[1] ^ temp[2] ^ mul(0x02, temp[3]));
    }
    
}


void mix_column_inv(uint8_t *state) {
    uint8_t temp[4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            temp[j] = state[i * 4 + j];
        }
        state[i * 4] = (uint8_t)(mul(0x0e, temp[0]) ^ mul(0x0b, temp[1]) ^ mul(0x0d, temp[2]) ^ mul(0x09, temp[3]));
        state[i * 4 + 1] = (uint8_t)(mul(0x09, temp[0]) ^ mul(0x0e, temp[1]) ^ mul(0x0b, temp[2]) ^ mul(0x0d, temp[3]));
        state[i * 4 + 2] = (uint8_t)(mul(0x0d, temp[0]) ^ mul(0x09, temp[1]) ^ mul(0x0e, temp[2]) ^ mul(0x0b, temp[3]));
        state[i * 4 + 3] = (uint8_t)(mul(0x0b, temp[0]) ^ mul(0x0d, temp[1]) ^ mul(0x09, temp[2]) ^ mul(0x0e, temp[3]));
    }
}

//je suis vraiment vraiment pas sur de cete fonctoin mais j'ai pas le choix là 
void add_round_key(uint8_t *state, uint8_t *round_key, int round) {
    for (int i = 0; i < 4; i++) {
        state[i] ^= round_key[round * 4 + i];
    }
}


//on tourne
void RotWord(uint8_t *word) {
    uint8_t temp = word[0];
    for (int i = 0; i < 3; i++) {
        word[i] = word[i + 1];
    }
    word[3] = temp;
    
}

//on fait la Sbox du mot
void subWord(uint8_t *word) {
    for (int i = 0; i < 4; i++) {
        word[i] = get_sbox_value(word[i]);
    }
}

//plutôt que de faire un xor {02}^-1 je prend direct dans la liste des valeurs possible(trouvé sur le net)
unsigned char Rcon[255] = { 

    };


//comme dans le papier
void KeyExpansion(uint8_t* key, uint8_t* expanded_key) {
    int current_size = 0;
    int wanted_size = Nb * (Nr + 1);
    int rcon_index = 1;
    uint8_t temp[4];
    for (int i =0; i<4; i++){
       expanded_key[i] = key[i];
       current_size++;
    }
    
    while(current_size < wanted_size) {
        for (int i = 0; i < 4; i++) {
            temp[i] = expanded_key[current_size - 4 + i];
        }
        if (current_size % Nk == 0) {
            RotWord(temp);
            subWord(temp);
            temp[0] ^= Rcon[rcon_index];
            rcon_index++;
        } else if (Nk > 6 && current_size % Nk == 4) {
            subWord(temp);
        }
        for (int i = 0; i < 4; i++) {
            expanded_key[current_size] = expanded_key[current_size - Nk * 4] ^ temp[i];
            current_size++;
        }
        
    }
}

void cipher_block(uint8_t* input, uint8_t* output, uint8_t* expanded_key) {
    uint8_t state[16];
    for (int i = 0; i < 16; i++) {
        state[i] = input[i];
    }
    
    add_round_key(state, expanded_key, 0);

    for (int round = 1; round < Nr; round++) {
        sub_bytes(state);
        shift_row(state);
        mix_column(state);
        add_round_key(state, expanded_key, round);
    }

    sub_bytes(state);
    shift_row(state);
    add_round_key(state, expanded_key, Nr);

    for (int i = 0; i < 16; i++) {
        output[i] = state[i];
    }
}

void decipher_block(uint8_t* input, uint8_t* output, uint8_t* expanded_key) {
    uint8_t state[16];
    for (int i = 0; i < 16; i++) {
        state[i] = input[i];
    }
    add_round_key(state, expanded_key, Nr);
    for(int round = 1; round< Nr; round++){
        inv_shift_row(state);
        inv_sub_bytes(state);
        add_round_key(state, expanded_key, Nr-round);
        mix_column_inv(state);
    }
    inv_shift_row(state);
    inv_sub_bytes(state);
    add_round_key(state, expanded_key, 0);

    for (int i = 0; i<16; i++){
        output[i]= state[i];
    }

}